...
class POS:
    ...
    def __init__(self, root):
        ...
        tk.Button(self.root, text="今日營收", command=self.show_today_total).grid(row=19, column=9, padx=10, pady=5)
   ...
   #判斷是否選中單子
    ...
    #顯示今日營收
    def show_today_total(self):
        try:
            resp = requests.get(f"{self.API_BASE}/report/today-total")
            if resp.status_code != 200:
                messagebox.showerror("錯誤", resp.text)
                return

            data = resp.json()
            messagebox.showinfo("今日營收", f"{data['today_total']}")
        except Exception as e:
            messagebox.showerror("錯誤", str(e))
